			       ...
			       int priority, int file)
{
	...
	while (!list_empty(&l_hold)) {
		...
		if (page_referenced(page, 0, mz->mem_cgroup, &vm_flags)) {
			//Set PG_referenced to 0 when first referenced
		...
		else if((page->PG_referenced+=SHIFT) <= THRESHOLD ){
			printk(KERN_INFO"%ld:Found page not refereced in shrink_active_list, variable=%lu, not exceeded yet.\n",page->index,page->PG_referenced);
			list_add(&page->lru, &l_active);
				continue;
		}

		printk(KERN_INFO"%ld:Found page not refereced in shrink_active_list, variable=%lu, shrinked.\n",page->index,page->PG_referenced);
